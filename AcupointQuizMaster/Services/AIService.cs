...
using AcupointQuizMaster.Config;
...
namespace AcupointQuizMaster.Services
{
    ...
    public class AIService : IDisposable
    {
        ...
        public AIService()
        {
            ...
            // 初始化默认设置，优先使用配置的默认API密钥
            _apiKey = DefaultApiConfig.GetDefaultDeepSeekApiKey();
            _apiUrl = DefaultApiUrl;
            _modelName = DefaultModelName;
            _platform = AIPlatform.DeepSeek;
        }
            ...
            string acupointName, AcupointInfo acupointInfo, string forcedQuestionType)
        {
            try
            {
                ...
                var response = await CallAIAsync(systemMessage, request, responseJson: true, temperature: 0.35);
            ...
            string questionType, AcupointInfo acupointInfo)
        {
            try
            {
                ...
                var response = await CallAIAsync(systemMessage, request, responseJson: true, temperature: 0.0);
                var content = ExtractContent(response);
        ...
        /// 调用AI API - 支持多平台
        /// </summary>
        private async Task<dynamic> CallAIAsync(string systemMessage, object userContent, bool responseJson = true, double temperature = 0.2)
        {
            switch (_platform)
            {
                case AIPlatform.DeepSeek:
                case AIPlatform.OpenAI:
                case AIPlatform.Baichuan:
                case AIPlatform.ChatGLM:
                case AIPlatform.Custom:
                    return await CallOpenAICompatibleAsync(systemMessage, userContent, responseJson, temperature);
                
                case AIPlatform.Gemini:
                    return await CallGeminiAsync(systemMessage, userContent, temperature);
                
                case AIPlatform.Claude:
                    return await CallClaudeAsync(systemMessage, userContent, temperature);
                
                case AIPlatform.Qwen:
                    return await CallQwenAsync(systemMessage, userContent, temperature);
                
                default:
                    throw new NotSupportedException($"不支持的AI平台: {_platform}");
            }
        }

        /// <summary>
        /// 调用OpenAI兼容的API（DeepSeek, OpenAI, 百川, ChatGLM等）
        /// </summary>
        private async Task<dynamic> CallOpenAICompatibleAsync(string systemMessage, object userContent, bool responseJson, double temperature)
        {
            var messages = new[]
            {
        ...
        /// 调用Google Gemini API
        /// </summary>
        private async Task<dynamic> CallGeminiAsync(string systemMessage, object userContent, double temperature)
        {
            var url = _apiUrl.Replace("{model}", _modelName) + $"?key={_apiKey}";
            ...
            var payload = new
            {
                contents = new[]
                {
                    new
                    {
                        parts = new[]
                        {
                            new { text = systemMessage + "\n\n" + JsonConvert.SerializeObject(userContent, Formatting.None) }
                        }
                    }
                },
            ...
            if (!response.IsSuccessStatusCode)
            {
                throw new Exception($"Gemini API调用失败: {response.StatusCode} - {responseText}");
            }

            var geminiResponse = JsonConvert.DeserializeObject<dynamic>(responseText);
            
            // 转换为OpenAI格式
            var geminiText = geminiResponse?.candidates?[0]?.content?.parts?[0]?.text?.ToString() ?? "";
            return new
            {
                choices = new[]
                {
                    new
                    {
                        message = new { content = geminiText }
                    }
                }
            };
        }
        ...
        /// </summary>
        private async Task<dynamic> CallClaudeAsync(string systemMessage, object userContent, double temperature)
        {
            var payload = new
            {
                model = _modelName,
            ...
            if (!response.IsSuccessStatusCode)
            {
                throw new Exception($"Claude API调用失败: {response.StatusCode} - {responseText}");
            }

            var claudeResponse = JsonConvert.DeserializeObject<dynamic>(responseText);
            
            // 转换为OpenAI格式
            var claudeText = claudeResponse?.content?[0]?.text?.ToString() ?? "";
            return new
            {
                choices = new[]
                {
                    new
                    {
                        message = new { content = claudeText }
                    }
                }
            };
        }
        ...
        /// 调用阿里通义千问API
        /// </summary>
        private async Task<dynamic> CallQwenAsync(string systemMessage, object userContent, double temperature)
        {
            var payload = new
            {
                model = _modelName,
            ...
            if (!response.IsSuccessStatusCode)
            {
                throw new Exception($"通义千问API调用失败: {response.StatusCode} - {responseText}");
            }

            var qwenResponse = JsonConvert.DeserializeObject<dynamic>(responseText);
            
            // 转换为OpenAI格式
            var qwenText = qwenResponse?.output?.choices?[0]?.message?.content?.ToString() ?? "";
            return new
            {
                choices = new[]
                {
